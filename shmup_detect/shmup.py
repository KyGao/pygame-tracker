...
detectron = handdetect.Detectron()  # init detectron
...
while cap.isOpened():
    ...
    if not onTracking:
        bbox = detectron.detect(frame)
        if not len(bbox):
            screen.blit(background,background_rect)
            draw_text(screen,"Only one hand in front of camera", 22, WIDTH / 2, HEIGHT / 4)
            pygame.display.flip()
        else:   
            ix, iy, w, h = bbox[0], bbox[1], bbox[2], bbox[3]
